...
# id, name, artist, album, track number, duration, position, state, artwork url.
# The script appends a tenth field, "END".
_FIELD_COUNT = 9
...
class AppleScriptSource(TrackSource):
    ...
    def get_playback(self) -> Optional[Dict]:
        ...
        # The trailing "END" is what proves the payload is whole: without it an
        # empty artwork url and a truncated read look alike.
        if len(parts) != _FIELD_COUNT + 1 or parts[-1] != "END":
