...
(the raw thumbnail is cached to a temp file per track, served as ``file://``),
and no release date (year tag falls back to "0000"). Position updates only
when the app pushes one, not on a clock, so the progress figure steps rather
than ticks; and for ~0.5s after a skip it updates fields independently, sometimes
...
class SmtcSource(TrackSource):
    ...
    def __init__(self, cfg):
        ...
        self._art_urls: Dict[str, Optional[str]] = {}
    ...
    def connect(self) -> None:
        ...
        try:
            self._manager = self._loop.run_until_complete(self._request_manager())
    ...
    async def _get_playback_async(self) -> Optional[Dict]:
        # The manager from connect() serves every poll; the session list is
        # what changes, and get_sessions() re-reads it each time.
        session = self._find_spotify_session(self._manager)
        if session is None:
            return None
        ...
        track_id = "local:" + digest
        art_url = await self._art_url(props.thumbnail, digest)
    ...
    async def _art_url(self, thumbnail, digest: str) -> Optional[str]:
        """The ``file://`` url of one track's cover, written once per track.

        The url is read back at tag time, which happens after the next track
        has started, so each track keeps its own file rather than sharing one
        that the next poll would overwrite with the next track's art.
        """
        if digest not in self._art_urls:
            self._art_urls[digest] = await self._save_thumbnail(thumbnail, digest)
        return self._art_urls[digest]

    async def _save_thumbnail(self, thumbnail, digest: str) -> Optional[str]:
        ...
        try:
            ...
            path = self._tmp_dir / f"{digest}.img"
